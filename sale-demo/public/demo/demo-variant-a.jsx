// demo-variant-a.jsx — 独立可滚动版本，用于埋点 Demo
// 修改自 variant-a.jsx：去掉固定高度和 overflow:hidden，导航栏固定，CTA 加 data-cta 属性
  ...
  return (
    <header style={{
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        ...
        <button data-cta="apply-trial" style={{
...
function AHome({ setPage, setActive }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      {/* Hero */}
      <section style={{ padding: '64px 56px 48px', display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 56, maxWidth: 1280, width: '100%', margin: '0 auto', boxSizing: 'border-box' }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 36 }}>
            <button data-cta="browse-market" onClick={() => setPage('marketplace')} style={{
              padding: '14px 26px', background: A.ink, color: A.bg, border: 'none',
              ...aMono, fontSize: 12, letterSpacing: 1, cursor: 'pointer',
            }}>浏览市场 →</button>
            <button data-cta="view-cases" onClick={() => setPage('cases')} style={{
        ...
        <div style={{ borderLeft: `1px solid ${A.rule}`, paddingLeft: 40 }}>
          ...
          {window.AGENTS.slice(0, 5).map((a) => (
            <div key={a.id} data-cta="agent-item" onClick={() => { setActive(a); setPage('detail'); }} style={{
      ...
      <section style={{
        ...
        maxWidth: 1168, width: 'calc(100% - 112px)',
      }}>
      ...
      {/* Featured agents */}
      <section style={{ padding: '40px 56px 60px', maxWidth: 1280, width: '100%', margin: '0 auto', boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 18 }}>
          <div style={{ ...aMono, fontSize: 10, color: A.mute, textTransform: 'uppercase' }}>本周精选 · CURATED</div>
          <a data-cta="view-all" onClick={() => setPage('marketplace')} style={{ ...aMono, fontSize: 11, cursor: 'pointer' }}>查看全部 320 个 →</a>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
          {window.AGENTS.slice(0, 4).map((a) => (
            <div key={a.id} data-cta="agent-card" onClick={() => { setActive(a); setPage('detail'); }} style={{
              ...
              <div style={{ fontSize: 12, color: A.mute, marginTop: 10, lineHeight: 1.5, minHeight: 50 }}>{a.desc}</div>
...
function AMarket({ setPage, setActive }) {
  ...
  return (
    <div style={{ display: 'flex', maxWidth: 1280, width: '100%', margin: '0 auto', minHeight: 'calc(100vh - 64px)' }}>
      <aside style={{ width: 240, padding: '32px 28px', borderRight: `1px solid ${A.rule}`, flexShrink: 0 }}>
        ...
        {['全部', ...window.CATEGORIES].map(c => (
          <div key={c} data-cta="category-filter" onClick={() => setCat(c)} style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {list.slice(0, 6).map(a => (
            <div key={a.id} data-cta="agent-card" onClick={() => { setActive(a); setPage('detail'); }} style={{
              ...
              <div style={{ fontSize: 12, color: A.mute, marginTop: 12, lineHeight: 1.55, minHeight: 36 }}>{a.desc}</div>
    ...
    <div style={{ display: 'flex', maxWidth: 1280, width: '100%', margin: '0 auto', minHeight: 'calc(100vh - 64px)' }}>
      <div style={{ flex: 1, padding: '40px 56px' }}>
        <div style={{ ...aMono, fontSize: 10, color: A.mute, marginBottom: 14 }}>
          <span data-cta="back-to-market" onClick={() => setPage('marketplace')} style={{ cursor: 'pointer' }}>市场</span> &nbsp;/&nbsp; {a.category} &nbsp;/&nbsp; {a.name}
        </div>
      ...
      <aside style={{ width: 380, borderLeft: `1px solid ${A.rule}`, padding: 40, background: A.panel, flexShrink: 0 }}>
        ...
        <button data-cta="activate-agent" style={{
        ...
        <button data-cta="book-consult" style={{
        ...
        <div style={{ marginTop: 22, paddingTop: 18, borderTop: `1px solid ${A.rule}` }}>
          ...
          <pre style={{ ...aMono, fontSize: 10.5, lineHeight: 1.55, background: A.bg, padding: 12, margin: 0, color: A.ink, overflowX: 'auto' }}>{`POST /v1/agents/${a.id}/run\nAuthorization: Bearer •••\n{\n  "input": "...",\n  "stream": true\n}`}</pre>
        </div>
      </aside>
    </div>
  );
...
function ACases({ setPage }) {
  return (
    <div style={{ padding: '40px 56px 60px', maxWidth: 1280, width: '100%', margin: '0 auto', boxSizing: 'border-box' }}>
...
function VariantA() {
  ...
  return (
    <div style={aWrap}>
      <ANav page={page} setPage={handleSetPage} />
      {page === 'home'        && <AHome setPage={handleSetPage} setActive={setActive} />}
      {page === 'marketplace' && <AMarket setPage={handleSetPage} setActive={setActive} />}
      {page === 'detail'      && <ADetail active={active} setPage={handleSetPage} />}
      {page === 'cases'       && <ACases setPage={handleSetPage} />}
    </div>
  );
}
